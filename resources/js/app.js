...
var app = new Vue({
  ...
  {
    setSumData() {
      //alert(vardump(this.workerdays.datekey));
    ...
    getBaseDataWitId: function (pardata = 0) {
      return {
        id: pardata, month: this.month, year: this.year, cegid: this.cegid, workerids: this.workerids,
      ...
      axios.post(this.host + this.baseroute + task, data)
        .then(response => {
          ...
          if (task == 'getbasedata') {
           ...
           // this.actTimetype_id = Object.keys(this.timetypes)[0].id; //timetypetype selectlist actuális értéke ()()
