...
$data['times']= $times ;
 ...
 // return var_dump($timesob->with('worker')->where([ ['datum', '>=', $start],['datum', '<=', $end],['worker_id', '=', 5]])->selectRaw('sum(hour)')->get()->toarray() );
    ...
    {
        $stored= new Storedi();
        $data['storeds']=$stored->getStoredsData($year,$month,$cegid);
        return view('MocalendarVue.admin.calendarAdminStored', compact('data'));
    } 
    public function getStoreds($year=null,$month=null,$cegid='worker')
    {
        $stored= new Storedi();
        $data['storeds']=$stored->getStoredsData($year,$month,$cegid);
        $data['ev']=$year;
    ...
    /** 
     * csak kompatibilitás miatt , jsonStoredsData-t kell használni
     */
 public function getStoredsData($year = null, $month = null, $cegid = 'worker')
    {
